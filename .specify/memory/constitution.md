...
Branches follow one of two shapes, and no others:

* `<type>/<short-kebab-description>` for work that is not a Spec Kit feature,
  where type is one of `feat`, `fix`, `docs`, `refactor`, `test`, `chore` or
  `ci` — for example `docs/tech-stack`, `fix/sanitizer-jwt-pattern`.
* `<NNN>-<short-kebab-description>` for Spec Kit features, named after the
  feature directory `/speckit-specify` creates and reported by it as
  `BRANCH_NAME` — for example `001-chatgpt-import`. Spec Kit creates the branch
  itself only where its `before_specify` hook is installed; where it is not, the
  branch MUST be created with that exact name. These names MUST NOT be renamed:
  the name mirrors the feature directory under `specs/`, and the workflow
  scripts resolve one from the other.

Names are lowercase ASCII, hyphen-separated, and describe the change rather than
the person making it. Every branch starts from an up-to-date `origin/main`, and
carries the work of exactly one pull request.

Rationale: the branch name is what links a change to its specification and to
its pull request, and Spec Kit reads it as data rather than decoration.
...
**Version**: 1.2.1 | **Ratified**: 2026-09-12 | **Last Amended**: 2026-09-12
