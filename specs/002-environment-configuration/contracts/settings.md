...
| Missing `.env` loads; unreadable or malformed `.env` raises | FR-003, edge case |
...
| `.env.example` assigns no usable credential | SC-006 |
| `.env` ignored and `.env.example` tracked by version control | FR-011 |
| No module outside the settings module reads the environment | FR-001, SC-008 |
