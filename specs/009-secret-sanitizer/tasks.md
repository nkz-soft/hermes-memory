...
- [X] T001 [P] Write `tests/structure/test_sanitization_imports.py`: every `.py` under
...
- [X] T002 Write `tests/unit/test_redaction_scanner.py` first and observe it fail. It uses a small
...
- [X] T003 Implement `src/hermes_memory/sanitization/patterns.py`: the frozen `Pattern` record with
...
- [X] T004 Implement `src/hermes_memory/sanitization/scanner.py`: `Span` (half-open bounds,
...
- [X] T005 [US1] Create `tests/synthetic/secrets.py`: one synthesized sample per §13 category, each
...
- [X] T006 [P] [US1] Write `tests/unit/test_redaction_patterns.py` first and observe it fail: one
...
- [X] T007 [P] [US1] Write `tests/unit/test_pattern_sanitizer.py` first and observe it fail: the
...
- [X] T008 [P] [US1] Write `tests/structure/test_sanitizer_covers_the_model.py` first and observe it
...
- [X] T009 [P] [US1] Write `tests/contracts/test_pattern_sanitizer_passes_the_contract.py` first and
...
- [X] T010 [US1] Fill the value-shaped patterns in `src/hermes_memory/sanitization/patterns.py`:
...
- [X] T011 [US1] Fill the keyed patterns: API key as assignment, header and query parameter (RC-1);
...
- [X] T012 [US1] Fill the delimited-block pattern: the PEM envelope from `-----BEGIN … PRIVATE
...
- [X] T013 [US1] Implement `src/hermes_memory/sanitization/pattern_sanitizer.py`:
...
- [X] T014 [US1] Export `PatternSecretSanitizer` from
...
- [X] T015 [US2] Write `tests/unit/test_redaction_context.py` first and observe it fail: §13's
...
- [X] T016 [US2] Fix whatever T015 finds in the value groups of `patterns.py` — a group that
...
- [X] T017 [US3] Write `tests/unit/test_redaction_report.py` first and observe it fail: counts per
      ...
      report (RR-8, SC-005)
- [X] T018 [US3] Write `tests/unit/test_sanitizer_is_quiet.py` first and observe it fail: sanitizing
...
- [X] T019 [US3] Make T017 and T018 pass. Expected to be assertions about existing behaviour rather
...
- [X] T020 [US4] Write `tests/unit/test_redaction_false_positives.py` first and observe it fail:
...
- [X] T021 [US4] Tighten `patterns.py` until T020 passes without regressing T006 or T015: minimum
...
- [X] T022 [US5] Extend `tests/integration/test_pipeline_from_fakes.py`: compose
...
- [X] T023 [US5] In the same file, add the twin that proves the guard bites: the same composition
...
- [X] T024 [P] Write `tests/unit/test_redaction_cost.py`: each pattern finishes within a time bound
...
- [X] T025 [P] Add a canary test to `tests/unit/test_redaction_patterns.py`: the credential-shaped
...
- [X] T026 Run [quickstart.md](./quickstart.md) end to end and correct any step whose command,
      count or expected output does not match what the code does
- [X] T027 `uv run pytest`, `uv run ruff check .`, `uv run ruff format --check .` — all green, with
      no test skipped that this feature was supposed to run
- [X] T028 Re-read [plan.md](./plan.md)'s Constitution Check against the code as written, and amend
...
Every task above is done. Two are worth recording as having changed nothing, because a task that
found nothing is different from a task that was skipped:

* **T016** (fix what US2 finds in the value groups) — nothing to fix. §13's example, the `.env` key,
  the PEM markers, the connection-string host and the Secret manifest's keys all survived on the
  first run of `test_redaction_context.py`, so Phase 3's groups were already cut in the right place.
* **T021** (tighten the patterns until the false-positive suite passes) — nothing to tighten. The
  suite of `test_redaction_false_positives.py` passed against the table as Phase 3 left it, with the
  minimum lengths, the value alphabets, the `kind: Secret` guard and RR-7's rejection already in
  place.

One deviation from the plan, made while implementing and recorded here rather than left implicit:
`Pattern` gained a `requires` field — a guard expression the surrounding text must satisfy — because
a regular expression cannot look arbitrarily far behind its own match, and RC-13 needs the
`kind: Secret` line to be somewhere above the `data:` value. `data-model.md` was corrected to match.
